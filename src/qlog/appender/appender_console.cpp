...
bool appender_console::console_buffer::fetch_and_remove(
    type_func_ptr_console_buffer_fetch_callback callback, const void* pass_through_param
)
{
...
void appender_console::default_console_output(
    serialization::log_level level, const char* text, int32_t len
)
{
    if (!text || len <= 0)
    {
...
void appender_console::log_impl(const entry_runtime_view& view)
{
    ...
    else
    {
        default_console_output(
            level, log_entry_cache_.c_str(), static_cast<int32_t>(log_entry_cache_.size())
        );
    }
...
void appender_console::on_log_item_recovery_begin(entry_runtime_view&)
{
    default_console_output(
        ...
        RECOVER_START,
        static_cast<int32_t>(std::strlen(RECOVER_START))
    );
}
...
{
    default_console_output(
        serialization::log_level::info, RECOVER_END, static_cast<int32_t>(std::strlen(RECOVER_END))
    );
}
