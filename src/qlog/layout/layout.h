...
struct format_info
{
    ...
    char grouping_char = '\0'; // '\0'=无, ','=千位逗号, '_'=千位下划线  规范位置:
                               // [[fill]align][sign][#][0][width][grouping][.precision][type]
    fmt_type type = fmt_type::none; // 类型标识符

    bool has_format() const noexcept
    {
        return align != align_type::none || sign != sign_type::minus || alt_form || zero_pad ||
               width > 0 || precision >= 0 || type != fmt_type::none;
    }
};
...
public:
    ...
    [[nodiscard]] std::string_view do_layout(
...
private:
    ...
    // 返回解析消耗的字符数
    static int parse_format_spec(std::string_view spec, format_info& out) noexcept;
    ...
    // 有符号整数
    size_t insert_integral_signed(int64_t val, const format_info& fi) noexcept;
    ...
    size_t insert_str_utf16(std::string_view raw_bytes, const format_info& fi) noexcept;

    size_t insert_str_utf32(std::string_view raw_bytes, const format_info& fi) noexcept;
    // 指针
    ...
    void
    fill_and_alignment(const char* content, size_t content_len, const format_info& fi) noexcept;

    // 时间戳格式化
    size_t insert_timestamp(uint64_t timestamp_ms) noexcept;

    void scan_params() noexcept;
    // 参数分发
    // 根据param_type 调用对应的inset_* 函数
    // 返回该参数在params_buf占用的字节数（用于skip）
    size_t insert_param(const uint8_t* param_ptr, const format_info& fi) noexcept;
